...
setwd("E:/Academic/Thesis/Serum and Plasma Metabolomic/serum/")
...
system.time(results <- rfe(features[,1:13], as.factor(class[,1]), sizes=c(1:10), rfeControl=control))
...
bestMetabols = rfeRanked[1:5]

#write.csv(bestMetabols, file="rfeSerumMetabols.csv")

#save(bestMetabols, file="rfeSerumMetabols.rdata")

load("rfeSerumMetabols.rdata")
...
svm_model_after_tune <- svm(classTrain ~ ., data = featuresTrain, kernel="radial", cost=1, gamma=2)
